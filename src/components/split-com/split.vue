<template>
    <div :class="getClass" class="ui-split">
        <slot />
        <div v-for="i in itemLength" v-if="i>1" :key="i" :style="getDashStyle(i)" class="ui-split-dash" @mousedown.stop="handleMouseDown(i, $event)" />
    </div>
</template>
<script>
...
export default {
    ...
    computed: {
        ...
        fristItem() {
            if (this.moveIndex) {
                let fristItem = this.items.filter((item)=>{
        ...
        secondItem() {
            if (this.moveIndex) {
                let secondItem = this.items.filter((item)=>{
